...
void loop() {
  float iaq = boschMgr->Next(g_debug);  // polling, should be below 3 sec. Data
                                        // are povided every 3000ms
  ...
  bool connFailed = uploader->SendData(
      boschMgr->GetData(),
      g_debug);  // collect data only if the sensor was read. Upate to server
                 // after collecting 3k of data.
  if (connFailed) {
