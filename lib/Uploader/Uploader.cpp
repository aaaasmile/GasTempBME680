...
bool Uploader::SendData(String dataLine, bool debug) {
  ...
  if (!g_connected) {
    if (debug) {
      Serial.printf("device is not connected to WLAN, avoid send\n");
    }
    Setup();
    g_dataBuff = dataLine;
    return false;
  ...
  httpsClient
      .setInsecure();  // should be setTrustAnchors() // Ref:
  ...
  if (r == max_retry) {
    if (debug) {
      Serial.println(
          "Connection failed, discard this buffer and hope for the next "
          "send");
    }
    g_dataBuff = "";
    return true;
